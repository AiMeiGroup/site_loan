...
class PersonManage
{
	 ...
	 *新增数据
	 */
	public static function add($user_id, $name, $id_type, $id_number, $regions, $marriage_status, $children, $education,$salary,$insurance,$insurance_num,$house_condition,$is_buy_car,$is_overdue)
	{
		try {
			...
			$arr["regions"] = $regions;
			$arr["marriage_status"] = $marriage_status;
			$arr["children"] = $children;
			$arr["education"] = $education;
			$arr["salary"] = $salary;
			$arr["insurance"] = $insurance;
			$arr["insurance_num"] = $insurance_num;
			$arr["house_condition"] = $house_condition;
			$arr["is_buy_car"] = $is_buy_car;
			$arr["is_overdue"] = $is_overdue;
			$arr["created_time"] = date('Y-m-d H:i:s', time());
			$arr["modified_time"] = date('Y-m-d H:i:s', time());
			$id = \Lavender\IdMaker::make('person');
			return Dao\PersionTable::instance()->add($id, $arr);
