...
(function ($) {
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function () {
            ...
            //外层
            g.wrapper = g.inputText.wrap('<div class="l-text l-text-combobox" style="display:inline-block;"></div>').parent();
            g.wrapper.append('<div class="l-text-l"></div><div class="l-text-r"></div>');
            g.wrapper.append(g.link);
            //添加个包裹，
            g.textwrapper = g.wrapper.wrap('<div class="l-labeltext"></div>').parent();
        ...
        _setLabel: function (value) {
            ...
            if (!p.label) return;
            if (!g.labelwrapper) {
                g.labelwrapper = g.wrapper.parent();
                ...
                g.labelwrapper.width(p.labelWidth + p.width + 4);
            }
            else {
        ...
        _setValue: function (value, text) {
            ...
            if (p.tree) {
                ...
                if (value != null) {
                    ...
                    for (var i = 0; i < targetdata.length; i++) {
                        $("table.l-table-checkbox tr[value=" + targetdata[i] + "] :checkbox", g.selectBox).each(
                        function () {
                            $(this).prev().addClass("l-checkbox-checked");
                            this.checked = true;
                        });
                    }
                }
            ...
            if (grid.checkbox) {
                ...
                g.grid.bind('CheckAllRow', function (checked) {
                    $(g.grid.rows).each(function (rowdata) {
                        onCheckRow(checked, g.grid.rows[rowdata]);
                    });
                    onGridSelect();
                });
                ...
                //罗磊增加回发记忆功能
                //回发后初始化
                $(g.grid.rows).each(function (index) {
                    var rowdata = g.grid.rows[index];
                    if (p.value) {
                        for (var i in p.value.split(p.split)) {
                            if (p.value.split(p.split)[i] == rowdata[p.valueField]) {
                                selecteds.push(rowdata);
                            }
                        }
                    }
