...
$.ligerDefaults.Form.editorBulider = function (jinput) {
    ...
    if (ltype == "radiolist") {   //单选列表
        jinput.ligerRadioList(options);
    }
    else if (ltype == "checkboxlist") {  //复选列表
        jinput.ligerCheckBoxList(options);
    }
    else if (ltype == "listbox") {  //列表框
        jinput.ligerListBox(options);
    }
    else if (ltype == "button") {   //按钮
        jinput.ligerButton(options);
    }
    else if (ltype == "textarea") {   //按钮
        jinput.ligerTextArea(options);
    }
    if (jinput.is("select")) {     //下拉框
        jinput.ligerComboBox(options);
    }
    else if (jinput.is(":password")) {   //密码框
    ...
    else if (jinput.is(":radio")) {   //这里可以注释。此控件很少用到
...
$.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
    ...
    _render: function () {
        ...
        //生成ligerui表单样式
        $("input,select,textarea,div[ltype]", jform).each(function () {
