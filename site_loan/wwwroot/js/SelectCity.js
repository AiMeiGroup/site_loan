...
Vcity.allCity = ['北京|beijing|bj', '上海|shanghai|sh', '重庆|chongqing|cq', '深圳|shenzhen|sz', '广州|guangzhou|gz', '杭州|hangzhou|hz',
    ...
    '朝阳|chaoyang|cy', '常德|changde|cd', '长白山|changbaishan|cbs', '赤峰|chifeng|cf', '大同|datong|dt', '大连|dalian|dl', '达县|daxian|dx', '东营|dongying|dy', '东莞|dongguan|dg', '大庆|daqing|dq', '丹东|dandong|dd',
    '大理|dali|dl', '敦煌|dunhuang|dh', '鄂尔多斯|eerduosi|eeds', '恩施|enshi|es', '福州|fuzhou|fz','佛山|fusan|fs', '阜阳|fuyang|fy', '贵阳|guiyang|gy',
    ...
    '嘉峪关|jiayuguan|jyg', '井冈山|jinggangshan|jgs', '济宁|jining|jn', '九江|jiujiang|jj', '佳木斯|jiamusi|jms','江门|jiangmen|jm', '济南|jinan|jn',
    ...
    '张家界|zhangjiajie|zjj', '舟山|zhoushan|zs', '郑州|zhengzhou|zz', '中卫|zhongwei|zw','中山|zhongsan|zs', '芷江|zhijiang|zj', '湛江|zhanjiang|zj'];
...
function ValidateCity(value) {
    if (value == "东莞" || value == "深圳" || value == "珠海" || value == "广州" ||value == "中山" || value == "江门" || value == "佛山") {
        return true;
    }
    else {
        return false;
        ...
        for (key in oCity) {
            ...
            for (var j = 0, k = sortKey.length; j < k; j++) {
                ...
                for (var i = 0, n = oCity[key][sortKey[j]].length; i < n; i++) {
                    str = '<a href="javascript:void(0);" class="'+(ValidateCity(oCity[key][sortKey[j]][i])?"":"changeGray")+'">' + oCity[key][sortKey[j]][i] + '</a>';
                    odda.push(str);
                }
    ...
    linkEvent: function () {
        ...
        for (var i = 0, n = links.length; i < n; i++) {
            links[i].onclick = function () {
                if(ValidateCity(this.innerHTML))
                {
